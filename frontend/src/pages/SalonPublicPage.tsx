...
const SalonPublicPage: FC = () => {
    ...
    const [serviceDetailView, setServiceDetailView] = useState<any>(null);
    ...
    // ============================================================
    // 🔥 Dynamic Open Graph / SEO Meta Tag Injection
    // يقوم بتحديث وسوم الصفحة بعد جلب بيانات الصالون
    // لكي يظهر الرابط بشكل احترافي على فيسبوك وواتساب وتويتر
    // ============================================================
    useEffect(() => {
        if (!salon) return;

        const salonName = salon.name || 'صالون تجميل';
        const salonDesc = salon.description
            ? salon.description
            : `احجز موعدك الآن في ${salonName} - خدمات تجميل احترافية عبر منصة O2OEG`;
        const salonImage = salon.logo_url || 'https://o2oeg.com/og-default.png';
        const salonUrl = window.location.href;

        // تحديث عنوان الصفحة في تاب المتصفح
        document.title = `${salonName} | O2OEG`;
        ...
        // Open Graph Tags (فيسبوك، واتساب، تيليجرام)
        setMeta('og:title', `${salonName} | احجز موعدك الآن`);
        setMeta('og:description', salonDesc);
        setMeta('og:image', salonImage);
        setMeta('og:url', salonUrl);
        setMeta('og:type', 'business.business');
        setMeta('og:site_name', 'O2OEG - منصة الصالونات الذكية');
        setMeta('og:locale', 'ar_EG');

        // Twitter Card Tags
        setMeta('twitter:card', 'summary_large_image', 'name');
        setMeta('twitter:title', `${salonName} | احجز موعدك الآن`, 'name');
        setMeta('twitter:description', salonDesc, 'name');
        setMeta('twitter:image', salonImage, 'name');

        // تنظيف عند مغادرة الصفحة
        return () => {
            document.title = 'O2OEG - AI Beauty SaaS';
        };
    }, [salon]);
    ...
    const fetchSalonData = async () => {
        try {
            ...
            let endpoint = `/salons/${id}/public`;
            
            if (isCustomDomain) {
                endpoint = `/salons/by-domain/${hostname}/public`;
            } else if (isTenantSubdomain) {
                endpoint = `/salons/by-domain/${subdomain}/public`;
            } else if (!id) {
                setError('عذراً، لم نتمكن من تحديد الصالون المطلوب.');
                setLoading(false);
                return;
            }
    ...
    return (
        <div className="min-h-screen bg-[#0A0A0C] text-white pb-20 overflow-x-hidden" dir="rtl">
                        ...
                        >
                            {services.map((service, idx) => (
                                <motion.div
                                    ...
                                    className="bg-white/5 border border-white/10 rounded-[2rem] overflow-hidden hover:border-cyan-500/30 transition-all flex flex-col group"
                                >
                                    {/* Service Image */}
                                    <div className="relative h-48 bg-gradient-to-br from-cyan-500/10 to-fuchsia-500/10 overflow-hidden">
                                        {service.image_url ? (
                                            <img
                                        ...
                                        <span className="absolute bottom-3 right-3 text-xl font-black text-white drop-shadow">{parseFloat(service.price).toLocaleString()} ج.م</span>
                                    </div>
                                    {/* Card Body */}
                                    <div className="p-5 flex flex-col flex-1">
                                        <h3 className="text-lg font-bold text-white mb-1">{service.name}</h3>
                                        <p className="text-sm text-white/40 mb-4 line-clamp-2">{service.description || 'احصل على أفضل تجربة عناية واحترافية مع خبرائنا.'}</p>
                                        ...
                                        <button
                                            onClick={() => setServiceDetailView(service)}
                                            className="mt-auto w-full py-3 bg-white/5 hover:bg-cyan-500 hover:text-black font-bold rounded-2xl transition-all border border-white/10 hover:border-transparent flex items-center justify-center gap-2 text-sm"
                                        >
                                            احجز الآن
            ...
            {/* Service Detail + Booking Full Page Overlay */}
            <AnimatePresence>
                {serviceDetailView && (
                    <motion.div
                        ...
                        className="fixed inset-0 z-[100] bg-[#0A0A0C] overflow-y-auto"
                        dir="rtl"
                    >
                        {/* Header */}
                        <div className="sticky top-0 z-10 bg-[#0A0A0C]/90 backdrop-blur-2xl border-b border-white/5 px-6 py-4 flex items-center gap-4">
                            <button onClick={() => setServiceDetailView(null)} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center hover:bg-white/10 transition-all">
                                <ArrowLeft size={20} />
                            </button>
                            <div>
                                <h2 className="text-lg font-black text-white">{serviceDetailView.name}</h2>
                        ...
                        <div className="max-w-2xl mx-auto px-6 py-10 space-y-8">
                            {/* Service Image */}
                            {serviceDetailView.image_url && (
                                <div className="rounded-3xl overflow-hidden h-72">
                                    <img src={serviceDetailView.image_url} alt={serviceDetailView.name} className="w-full h-full object-cover" />
                                </div>
                            )}
                            ...
                            <div className="bg-white/5 rounded-3xl p-6 space-y-4 border border-white/10">
                                <div className="flex justify-between items-center">
                                    <h3 className="text-2xl font-black text-white">{serviceDetailView.name}</h3>
                                    <span className="text-2xl font-black text-cyan-400">{parseFloat(serviceDetailView.price).toLocaleString()} ج.م</span>
                                </div>
                                <p className="text-white/60 leading-relaxed">{serviceDetailView.description || 'احصل على أفضل تجربة عناية واحترافية مع خبرائنا في هذه الخدمة المميزة.'}</p>
                                <div className="flex gap-6 pt-2">
                                    <div className="flex items-center gap-2 text-sm text-white/40">
                                        <Clock size={16} className="text-cyan-400" />
                                        <span>{serviceDetailView.duration || '45'} دقيقة</span>
                                    </div>
                                    <div className="flex items-center gap-2 text-sm text-white/40">
                            ...
                            <div className="bg-white/5 rounded-3xl p-6 border border-white/10">
                                <h4 className="text-lg font-bold text-white mb-6">احجز موعدك الآن</h4>
                                <form onSubmit={(e) => { e.preventDefault(); setSelectedService(serviceDetailView); handleBooking(e).then(() => setServiceDetailView(null)); }} className="space-y-4">
                                    <div>
                                        <label className="text-xs font-bold text-white/40 mb-2 block">الاسم بالكامل</label>
                                        <input required type="text" value={bookingForm.customer_name} onChange={e => setBookingForm({...bookingForm, customer_name: e.target.value})} className="w-full bg-white/10 border border-white/10 rounded-2xl py-4 px-5 text-white outline-none focus:border-cyan-500 transition-all" placeholder="أدخل اسمك..." />
                                    </div>
                                    <div>
                                        <label className="text-xs font-bold text-white/40 mb-2 block">رقم الهاتف</label>
                                        <input required type="tel" value={bookingForm.customer_phone} onChange={e => setBookingForm({...bookingForm, customer_phone: e.target.value})} className="w-full bg-white/10 border border-white/10 rounded-2xl py-4 px-5 text-white outline-none focus:border-cyan-500 transition-all" placeholder="01xxxxxxxxx" />
                                    </div>
                                    <div>
                                        <label className="text-xs font-bold text-white/40 mb-2 block">تاريخ ووقت الموعد</label>
                                        <input required type="datetime-local" value={bookingForm.booking_date} onChange={e => setBookingForm({...bookingForm, booking_date: e.target.value})} className="w-full bg-white/10 border border-white/10 rounded-2xl py-4 px-5 text-white outline-none focus:border-cyan-500 transition-all" />
                                    </div>
                                    <div>
                                        <label className="text-xs font-bold text-white/40 mb-2 block">ملاحظات (اختياري)</label>
                                        <textarea value={bookingForm.notes} onChange={e => setBookingForm({...bookingForm, notes: e.target.value})} className="w-full bg-white/10 border border-white/10 rounded-2xl py-4 px-5 text-white outline-none focus:border-cyan-500 transition-all resize-none h-24" placeholder="أي طلبات أو ملاحظات خاصة..." />
                                    </div>
                                    <button type="submit" className="w-full py-5 bg-gradient-to-r from-cyan-500 to-fuchsia-600 text-white font-black rounded-2xl shadow-lg hover:scale-[1.02] active:scale-95 transition-all text-lg">
                                        تأكيد الحجز
                                    </button>
                                </form>
                            </div>
