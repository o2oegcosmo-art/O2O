...
type TabType = 'overview' | 'events' | 'crm' | 'pipeline' | 'ai' | 'marketing' | 'reports' | 'sales_team' | 'orders' | 'education' | 'catalog';
...
export default function CompanyDashboard() {
    ...
    const handleLogout = async () => {
        try {
            ...
            // 🛡️ Multi-Session Strategy: Only clear company-specific data
            localStorage.removeItem('o2oeg_token_company');
            localStorage.removeItem('o2oeg_user_company');
            sessionStorage.removeItem('o2oeg_token_company');
            sessionStorage.removeItem('o2oeg_user_company');
            ...
            <aside className={`fixed top-20 right-0 h-[calc(100vh-80px)] w-64 bg-[#0d0d10]/98 backdrop-blur-xl border-l border-white/8 z-40 transition-all duration-300 transform ${isSidebarOpen ? 'translate-x-0' : 'translate-x-full lg:translate-x-0'} flex flex-col shadow-2xl`}>
                <nav className="p-3 flex-1 overflow-y-auto flex flex-col gap-0.5">
                    ...
                    {[
                        ...
                        { tab: 'events', icon: Megaphone, label: 'الإعلانات والفعاليات', color: 'text-fuchsia-400', slug: 'events-management' },
                    ...
                    {[
                        { tab: 'orders', icon: ShoppingCart, label: 'الطلبات الواردة', color: 'text-cyan-400', slug: 'e-commerce' },
                        { tab: 'catalog', icon: Package, label: 'إدارة الكتالوج', color: 'text-cyan-400', slug: 'e-commerce' },
                    ].map(({ tab, icon: Icon, label, color, slug }) => {
                            ...
                            <button
                                ...
                                onClick={() => { 
                                    if (active) {
                                        ...
                                        toast.error("يرجى تفعيل خدمة المتجر الإلكتروني أولاً");
                                    }
                                }}
                                className={`flex items-center gap-3 w-full px-3 py-2.5 rounded-xl transition-all duration-300 text-sm font-bold border ${
                                    activeTab === tab
                    ...
                    {[
                        { tab: 'crm', icon: Users, label: 'إدارة الصالونات', color: 'text-cyan-400', slug: 'crm-system' },
                        { tab: 'sales_team', icon: MapPin, label: 'فريق المندوبين', color: 'text-green-400', slug: 'crm-system' },
                        { tab: 'pipeline', icon: GitBranch, label: 'خط المبيعات (Pipeline)', color: 'text-purple-400', slug: 'crm-system' },
                    ].map(({ tab, icon: Icon, label, color, slug }) => {
                        const active = isServiceActive(slug);
                        return (
                            <button
                                ...
                                onClick={() => { 
                                    if (active) {
                                        ...
                                        toast.error("يرجى تفعيل نظام إدارة العملاء (CRM) أولاً");
                                    }
                                }}
                                className={`flex items-center gap-3 w-full px-3 py-2.5 rounded-xl transition-all duration-300 text-sm font-bold border ${
                                    activeTab === tab
                    ...
                    {[
                        { tab: 'education', icon: GraduationCap, label: 'أثر التدريب (ROI)', color: 'text-amber-400', slug: 'crm-system' },
                        { tab: 'marketing', icon: MessageSquare, label: 'حملات الواتساب', color: 'text-green-400', slug: 'crm-system' },
                        { tab: 'ai', icon: Sparkles, label: 'AI Business Consultant', color: 'text-cyan-400', slug: 'crm-system' },
                    ].map(({ tab, icon: Icon, label, color, slug }) => {
                        const active = isServiceActive(slug);
                        return (
                            <button
