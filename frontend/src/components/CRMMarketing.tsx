...
const CRMMarketing: React.FC<Props> = ({ isLocked, onUpgrade }) => {
    ...
    const handleGenerateMsg = async () => {
        ...
        try {
            ...
            const errorMsg = err.response?.data?.error || 'فشل توليد الرسالة';
    ...
    const handleAction = async (id: number, action: 'start' | 'pause') => {
        try {
            const res = await api.post(`/whatsapp/campaigns/${id}/${action}`);
            if (action === 'start') {
                const data = res.data;
                if (data.sent > 0) {
                    toast.success(`✅ أُرسلت ${data.sent} رسالة بنجاح!\n${data.detail?.[0] || ''}`);
                } else if (data.failed > 0) {
                    toast.error(`❌ فشل الإرسال!\n${data.detail?.[0] || 'خطأ غير معروف'}`);
                } else if (data.detail) {
                    toast.error(`⚠️ ${data.detail[0]}`);
                } else {
                    toast.success('🚀 جاري الإرسال في الخلفية...');
                }
                loadCampaigns();
            } else {
                toast.success('⏸️ تم الإيقاف المؤقت');
                loadCampaigns();
            }
        } catch (err: any) {
            const msg = err?.response?.data?.detail?.[0] || err?.response?.data?.error || 'فشل تنفيذ العملية';
            toast.error(`❌ ${msg}`);
        }
    };
