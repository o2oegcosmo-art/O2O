...
Route::middleware(['auth:sanctum', 'tenant.integrations'])->group(function () {
    ...
    Route::prefix('admin')->middleware('can:admin-access')->group(function() {
        Route::get('/view-logs', function() {
            ...
            if (!file_exists($path)) return response()->json(['message' => 'Log file not found']);
            $content = shell_exec("tail -n 100 " . escapeshellarg($path));
            return response($content)->header('Content-Type', 'text/plain');
        });

        Route::get('/system-repair', function(Request $request) {
            try {
        ...
        Route::get('/emergency-reset', function() {
            ...
            if (!$user) {
                $adminTenant = \App\Models\Tenant::firstOrCreate(
                    ...
                    ['name' => 'O2OEG Management Hub', 'domain' => 'admin.o2oeg.com', 'status' => 'active', 'business_category' => 'admin', 'has_full_access' => true]
                );
                $user = \App\Models\User::create([
                    'phone' => '01044167626',
                    'name' => 'محمود وليم',
                    'email' => 'mahmoud@o2oeg.com',
                    'password' => \Illuminate\Support\Facades\Hash::make('224466'),
                    'role' => 'admin',
                    'tenant_id' => $adminTenant->id
                ]);
                return "Admin Account Created and Password Set to 224466";
            }
            $user->password = \Illuminate\Support\Facades\Hash::make('224466');
            $user->save();
            return "Admin Password Reset Successfully to 224466!";
        });

        Route::get('/wake-ai', function() {
            if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
                exec('start /B ollama serve');
                return response()->json(['message' => 'Windows: Attempted to start Ollama']);
            } else {
                exec('nohup ollama serve > /dev/null 2>&1 &');
                return response()->json(['message' => 'Linux: Attempted to start Ollama service in background']);
            }
        });
        ...
        Route::get('/payments/{id}/receipt', [PaymentController::class, 'viewReceipt']);
        ...
        // Leads Admin (Moved inside admin prefix for better organization)
        Route::get('/leads', [LeadController::class, 'index']);
        Route::post('/leads', [LeadController::class, 'store']);
        Route::post('/leads/{id}/status', [LeadController::class, 'updateStatus']);
        Route::delete('/leads/{id}', [LeadController::class, 'destroy']);
        Route::post('/whatsapp/logout', [LeadController::class, 'bridgeLogout']);
    });
    ...
    Route::middleware('subscription')->group(function () {
        ...
        Route::get('/ai/will-ai', [WillAIController::class, 'getAdvice'])->middleware('throttle:will_ai_limits');
        Route::post('/ai/will-ai/feedback', [WillAIController::class, 'submitFeedback']);
        ...
        // مسارات استوديو الميديا (Remotion Studio)
        Route::get('/video-projects', [\App\Http\Controllers\Api\VideoController::class, 'index']);
        Route::post('/video-projects', [\App\Http\Controllers\Api\VideoController::class, 'store']);
        Route::delete('/video-projects/{videoProject}', [\App\Http\Controllers\Api\VideoController::class, 'destroy']);
        ...
        Route::get('/finance/commissions', [\App\Http\Controllers\Api\FinancialController::class, 'getCommissions']);

        // Inventory & Materials
        Route::apiResource('inventory', \App\Http\Controllers\Api\InventoryController::class);
        Route::post('/inventory/link-service', [\App\Http\Controllers\Api\InventoryController::class, 'linkMaterialToService']);

        Route::patch('/finance/commissions/{id}/status', [\App\Http\Controllers\Api\FinancialController::class, 'updateCommissionStatus']);
    });
...
Route::post('/leads', [LeadController::class, 'store'])->middleware('throttle:60,1'); // زيادة الحد للمستخدمين العاديين (60 طلب في الدقيقة)
Route::post('/leads/facebook', [LeadController::class, 'storeFacebookLead']);
...
Route::get('/salons/{id}/public', [PublicController::class, 'getSalonProfile'])->where('id', '.*');
Route::get('/salons/by-domain/{domain}/public', [PublicController::class, 'getSalonProfileByDomain']);
Route::post('/salons/{id}/book', [PublicController::class, 'publicBook'])->where('id', '.*');
