    ...
    public function send($notifiable, Notification $notification)
    {
        ...
        if ($phoneNumberId === 'unofficial' || config('services.whatsapp.use_bridge')) {
            ...
            if ($payload['type'] === 'template') {
                $bridgeText = "Booking Update: " . ($payload['template']['name'] ?? 'General');
                ...
                $bridgeText = $payload['text']['body'] ?? 'Message';
            }

            $response = Http::withHeaders(['Authorization' => 'Bearer o2oeg_bridge_secret_2026_z8v9'])
                ->timeout(5) // Limit wait time to 5 seconds max
            ...
            if ($response->failed()) {
                Log::error("WhatsApp Bridge Notification Failure", [
