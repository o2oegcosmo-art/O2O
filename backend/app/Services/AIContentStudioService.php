...
class AIContentStudioService
{
    ...
    private function callGemini($prompt, Tenant $tenant = null, $feature = 'content_studio')
    {
        ...
        $fullPrompt = $prompt . $styleContext;
        if ($styleContext) {
            $fullPrompt .= "\n\nSTRICT REQUIREMENT: Ensure the output matches the 'Customer Style Sync' tone.";
        }

        $response = $this->aiRouter->route($fullPrompt, $tenant, $feature, true);

        if ($response['success']) {
            return $response['data'];
        }

        throw new \Exception('فشل الاتصال بمحركات الذكاء الاصطناعي (O2OEG Multi-Brain). يرجى التأكد من مفاتيح الـ API أو اتصال السيرفر.');
    }
    ...
    {
        Log::info("AIContentStudio: Generating WhatsApp Message for Tenant: {$tenant->name} using Goal: {$goal}");
        $prompt = "
